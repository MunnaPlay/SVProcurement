...
const L2L2Scanner = ({navigation}) => {
  ...
  const onSuccess=async(e)=>{
    ...
    if(filter.length==0)
    {
      if(e.data.charAt(0)!=2 || e.data.charAt(0)!='2')
      {
        alert('Error, Not L2 Scanner')
      }
      else
      {
  ...
  return (
    <View style={{flex:1}}>
      ...
      <TouchableOpacity onPress={() => navigation.navigate('L2L1Scanner')} style={{backgroundColor:'#F4A120',width:width/1.3,borderRadius:5,paddingLeft:10,margin:15,alignSelf:'center'}}>
